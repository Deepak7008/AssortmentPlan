...
const AttributeColumn = ({ title, items }: { title: string; items: { name: string; percent: number }[] }) => (
    <View className="flex-1 mx-1">
        <Text className="text-slate-400 text-[10px] font-bold uppercase mb-2 text-center">{title}</Text>
        {items.slice(0, 3).map((item, index) => (
            <View key={index} className="flex-row justify-between py-1 px-2 mb-1 bg-slate-800/30 rounded">
                <Text className="text-white text-xs" numberOfLines={1}>{item.name || '-'}</Text>
                <Text className="text-sky-400 text-xs font-bold">{item.percent}%</Text>
            </View>
        ))}
        {items.length < 3 && [...Array(3 - items.length)].map((_, i) => (
            <View key={`empty-${i}`} className="flex-row justify-between py-1 px-2 mb-1 bg-slate-800/30 rounded">
